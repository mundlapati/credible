...
bool BrowserApplication::first_run = true;
QStringList BrowserApplication::stringList = QStringList();
QDateTime BrowserApplication::prev_modified_time = QDateTime();

bool BrowserApplication::getAdStatus(QString& url)
{
	QFileInfo adsfilename("data/BlockAds.txt");
	QDateTime next_modified_time = adsfilename.lastModified();

	FILE* file = fopen("data/BlockAds.txt","r");
	char line[256];

	if (BrowserApplication::first_run)
	{
		BrowserApplication::prev_modified_time = adsfilename.lastModified();
		BrowserApplication::stringList.clear();
		BrowserApplication::first_run = false;
		while (fgets(line,sizeof(line),file))
		{
			if (*line == '#') continue;
			BrowserApplication::stringList << line;
		}
	}
	else if (next_modified_time > BrowserApplication::prev_modified_time)
	{
		BrowserApplication::prev_modified_time = next_modified_time;
		BrowserApplication::stringList.clear();
		BrowserApplication::first_run = false;
		while (fgets(line,sizeof(line),file))
		{
	...
	foreach(QString str, BrowserApplication::stringList)
	{
		if (url.contains(str.trimmed())) return true;
	}
	return false;
}
