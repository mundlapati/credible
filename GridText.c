...
void HText_endCredible(char *address)
{
    CTRACE((tfp, "CREDIBLE: Address: %s\n",address));
}
			       ...
			       int stop_before)
{
	...
	if(anchor_ptr->anchor != NULL)
	{
	dest = HTAnchor_followLink(anchor_ptr->anchor);
	address = HTAnchor_address(dest);
	if ((address != NULL) && (strlen(address) < 94)) 
	if (isHTTP_URL(address) || isHTTPS_URL(address)) 
	{
		if ((strlen(line_ptr->data) > 1))
		if ((cfp = LYAppendToTxtFile("output.txt")) != NULL)
		{
		fprintf(cfp, "%s %s\n",address,line_ptr->data);
		fclose(cfp);
		}
	}
	}
