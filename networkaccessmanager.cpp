...
QString url = req.url().toString().replace("http://",""); url.replace("https://","");
if (BrowserApplication::getAdStatus(url))
{
	qWarning() << "** BLOCKED ** --> " << url;
	QNetworkReply *reply = QNetworkAccessManager::createRequest(op, req, outgoingData);
	reply->abort(); return reply;
}
	if (m_adBlock)
	{
